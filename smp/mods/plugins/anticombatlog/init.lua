...
-- Fight start message
local FightMessage = "You are combat tagged!"

-- Fight end message
local EndFightMessage = "Your combat tag expired"
...
local function setplayerpvp(playername)
	if not anticombatlog[playername] then
		anticombatlog[playername] = 0
...
local function resettime(playername)
	if anticombatlog[playername] ~= nil then
		anticombatlog[playername] = 0
	end
end
...
local function on_combatlog(player, playername)
	...
	minetest.after((battletimeout - anticombatlog[playername]) or battletimeout, function(obj) 
    obj:remove()
	end, obj)
...
minetest.register_globalstep(function(dtime)
	for fighter,oldtime in pairs(anticombatlog) do
		if anticombatlog[fighter] then
			local newtime = oldtime + dtime
			anticombatlog[fighter] = newtime
...
minetest.register_on_leaveplayer(function(player)
	...
	if anticombatlog[name] and anticombatlog[name] > 0 then
